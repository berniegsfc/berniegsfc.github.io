...
;   </ul>
;
; @copyright Copyright (c) 2010-2025 United States Government as represented
...
pro SpdfCdasWsExample, $
    ...
    dataset = 'MMS1_FPI_BRST_L2_DES-DIST'
    timeInterval = obj_new('SpdfTimeInterval', $
                       '2015-09-01T12:11:14.000Z', '2015-09-01T12:20:54.000Z')

    inventory = cdas->getInventory(dataset, timeInterval = timeInterval)
    ...
    if obj_valid(inventory) then begin
        ...
        for i = 0, n_elements(intervals) -1 do begin
            ...
            print, '    ', intervals[i]->getIso8601Start(), $
                   ' to ', intervals[i]->getIso8601Stop()
        endfor

        exampleInterval = cdas->getExampleTimeInterval(dataset)

        if obj_valid(exampleInterval) then begin

            print, dataset, ' Example time interval:'
            print, '    ', exampleInterval->getIso8601Start(), $
                   ' to ', exampleInterval->getIso8601Stop()

        endif else print, 'Example interval not found'

    endif else print, 'getInventory(', dataset, ') failed'

    dataset = 'AC_H0_MFI'

    inventory = cdas->getInventory(dataset)
...
;            binData=binData, $
    ...
    print, 'getCdawlibData returned:'
    print, '  dataResults.magnitude.dat = ', dataResults.magnitude.dat

    ; requires IDL 8 and CDF 3.9 or higher
    dataResults = $
        cdas->getCdfDataHash( $
            timeInterval, dataset, varNames, $
            binData=binData, $
    ...
    if dataResults.IsEmpty() then begin

        print, 'getCdfDataHash returned nothing'
    endif else begin

        print, 'getCdfDataHash returned:'
        print, '  dataResults[''Variables'', ''Magnitude'', ''VarData''] = ', $
                dataResults['Variables', 'Magnitude', 'VarData']
    endelse
